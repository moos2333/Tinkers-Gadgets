...
public class ChainMaterialStats extends AbstractMaterialStats {
    public static final ChainMaterialStats UNKNOWN = new ChainMaterialStats(3, 4.0f, 0, 0.0f, 0.0f);
    public final int maxBounces;
    public final float bounceRange;
    public final int ammoBonus;
    public final float speedRate;
    public final float comboBonus;

    public ChainMaterialStats(int maxBounces, float bounceRange, int ammoBonus, float speedRate, float comboBonus) {
    ...
    private String formatComboBonus(float value) {
        String sign = value >= 0 ? "+" : "-";
        String percent = String.format("%.0f", Math.abs(value * 100));
        TextFormatting color = value > 0 ? TextFormatting.GREEN : (value < 0 ? TextFormatting.RED : TextFormatting.GRAY);
        return TextFormatting.GRAY + Util.translate("stat.chain.combo_bonus.name") + " " +
                color + sign + percent + "%" + TextFormatting.RESET;
    }
    ...
    public List<String> getLocalizedDesc() {
        return ImmutableList.of(
                ...
                Util.translate("stat.chain.combo_bonus.desc"),
